...
import by.bratukhin.shortener.model.AbstractDataObjectFields;
import by.bratukhin.shortener.model.ShortLink;
import by.bratukhin.shortener.model.ShortLinkFields;
...
class UrlShortenerServiceImpl implements UrlShortenerService {
    ...
    public Mono<ItemsPage<ShortLink>> getShortLinks(String nextCursor, int pageSize) {
        ...
        if (StringUtils.isNotBlank(nextCursor)) {
            criteria = Criteria.where(AbstractDataObjectFields.id).lessThan(nextCursor);
        }
        ...
        Query query = Query.query(criteria)
            ...
            .sort(Sort.by(AbstractDataObjectFields.id).descending());
    ...
    public Mono<ShortLink> getUrlMetadataByShortCode(String shortCode) {
        Argument.checkNotNullWithGenericMessage(shortCode, ShortLinkFields.shortCode);
